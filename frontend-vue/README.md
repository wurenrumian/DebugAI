...
**可复用组件**：
三个历史记录组件已封装为独立可复用组件，适用于班级管理等场景：
...
<script setup>
const handleViewDetails = ({ records, initialSubmission, type }) => {
  // 统一处理详情展示
  // records: 排序后的记录数组
  // initialSubmission: { problem_description, code }
  // type: 'debug' \| 'evaluate' \| 'recommend'
}
</script>
```
