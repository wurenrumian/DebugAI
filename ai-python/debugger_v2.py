...
"""
    ...
    def _create_round2_prompt(self, submission: CodeSubmissionV2) -> str:
        """第2轮：结合确认结果，指出问题点"""
        history_text = self._format_history(submission.dialogue_history, 2)
...
"""
    ...
    def _create_round3_prompt(self, submission: CodeSubmissionV2) -> str:
        """第3轮：提供debug要点"""
        history_text = self._format_history(submission.dialogue_history, 3)
...
"""
    ...
    def _create_round4_prompt(self, submission: CodeSubmissionV2) -> str:
        """第4轮：详细指导修改"""
        history_text = self._format_history(submission.dialogue_history, 4)
...
"""
    ...
    def _format_history(self, history: List[DialogueTurn], latest: int) -> str:
        """格式化对话历史"""
        if not history:
            return "无对话历史"
        
        history_text = "学生（第1轮）：前述题目要求及代码，请助手进行思路理解确认"
        for turn in history:
            role = "学生" if turn.role == "student" else "助手"
            if turn.round_number!=latest:
...
这是第2轮对话：指出问题点和薄弱点。
1. 请结合学生的思路确认结果和测试点通过信息，指出代码中的主要问题点。
2. 薄弱点识别：每一处问题点从以下规范的关键词中选取1个作为相应的薄弱点：

薄弱点关键词规范（必须使用以下关键词）：
1. 语法类：类型不匹配,头文件缺失,未声明变量,其他语法错误
2. 逻辑类：边界条件错误,循环条件错误,逻辑顺序错误,未考虑边界条件
3. 算法类：算法选择不当,递归深度过大
4. 内存类：数组越界,空指针访问,内存泄漏,栈溢出
5. 其他类：输出格式错误,文件操作错误,其他
